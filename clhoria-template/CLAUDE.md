...
**Auto-load**: `import.meta.glob` from `routes/{tier}/**/*.index.ts` - each module must `export default` the route instance

**Declarative Config**: `app.config.ts` uses `defineConfig()` to drive application assembly with tier-level middleware and OpenAPI settings

**Middleware Chain**: Ordered middleware pipeline in `create-app.ts` (Request ID → Logging → Security → Timeout → Rate Limit → Body Limit → Compression)

**Singleton System**: Process-level singletons in `src/lib/core/singleton.ts` (DB, Redis, Casbin, Logger) with automatic HMR cleanup
...
**Specialized Loggers**: Use `logger` (system), `operationLogger` (CRUD audit with `type: OPERATION`), `loginLogger` (`type: LOGIN`)
...
## Tier-Level Middleware System

**Tier Middleware Files**: `routes/{tier}/_middleware.ts` auto-loaded for each tier
**Conditional Middleware**: Use `defineMiddleware()` with `MiddlewareWithExcept` pattern for conditional execution
**Middleware Order**: Global middleware (`create-app.ts`) → Tier middleware (`_middleware.ts`) → Route-level middleware
...
**API Documentation**: Start server with `pnpm dev`, visit `http://localhost:9999/doc` for interactive Scalar UI docs

**Testing Structure**: Place tests in `__tests__/` directories alongside route modules, use Vitest with `.int.test.ts` suffix for integration tests
