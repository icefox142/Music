...
import emojisRouter from "@/routes/client/emojis/index";
...
function createEmojisApp() {
  return createTestApp()
    .use("/emojis/*", jwt({ secret: env.CLIENT_JWT_SECRET, alg: "HS256" }))
    .route("/", emojisRouter);
}
...
describe("client emojis routes", () => {
  ...
  describe("authentication", () => {
    it("should require authentication", async () => {
      const response = await client.emojis.$get(
        { query: {} },
        {},
      );
  ...
  describe("post /emojis - create emoji", () => {
    it("should validate required fields", async () => {
      const response = await client.emojis.$post(
        {
  ...
  describe("get /emojis/{id} - get emoji", () => {
    ...
    it("should validate UUID format", async () => {
      const response = await client.emojis[":id"].$get(
  ...
  describe("patch /emojis/{id} - update emoji", () => {
    ...
    it("should update emoji tags", async () => {
      ...
      const response = await client.emojis[":id"].$patch(
        {
          param: { id: emojiId },
          json: {
            tags: newTags,
  ...
  describe("delete /emojis/{id} - delete emoji", () => {
    it("should validate UUID format", async () => {
      const response = await client.emojis[":id"].$delete(
        { param: { id: "invalid-uuid" } },
        { headers: getAuthHeaders(clientToken) },
      );
    ...
    it("should delete emoji successfully", async () => {
      ...
      const createResponse = await client.emojis.$post(
        {
          json: {
            ...testEmoji,
      ...
      const deleteResponse = await client.emojis[":id"].$delete(
      ...
      const verifyResponse = await client.emojis[":id"].$get(
        { param: { id: deleteTestId } },
        { headers: getAuthHeaders(clientToken) },
      );
